...
export function usePokemon() {
    ...
    const filteredAndSortedPokemons = computed(() => {
        const filtered = pokemons.value.filter((p) => p.name.toLowerCase().includes(searchTerm.value.toLowerCase()) ||
    ...
    const selectPokemon = async (pokemonIdentifier) => {
        const url = typeof pokemonIdentifier === 'string' && pokemonIdentifier.startsWith('http')
            ? pokemonIdentifier
    ...
    const navigatePokemon = async (direction) => {
        ...
        const currentIndex = filteredAndSortedPokemons.value.findIndex((p) => Number(p.url.split('/').filter(Boolean).pop()) === currentId);
