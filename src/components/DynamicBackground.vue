...
onMounted(() => {
  ...
  const color = ref<[number, number, number]>([0.6, 0.8, 1.0])
  const mode = ref<number>(0) // 0 water, 1 fire, 2 grass

  // Initialize library with our fragment shader and target DOM container
  startShader({ domElement: canvas.value.parentElement as HTMLElement, fragShader: fragmentShader })
    ...
    shaders: {
      main: {
        uniforms: {
          ...
          ) => gl.uniform2f(loc, g.width, g.height),
          u_time: (gl: WebGLRenderingContext | WebGL2RenderingContext, loc: WebGLUniformLocation) =>
            gl.uniform1f(loc, performance.now() / 1000),
          u_color: (
            gl: WebGLRenderingContext | WebGL2RenderingContext,
            loc: WebGLUniformLocation,
          ) => gl.uniform3f(loc, color.value[0], color.value[1], color.value[2]),
          u_mode: (gl: WebGLRenderingContext | WebGL2RenderingContext, loc: WebGLUniformLocation) =>
            gl.uniform1i(loc, mode.value),
        },
      },
    },
  })
  ...
  watch(
    ...
    (newType) => {
      if (newType) {
        ...
        if (newType === 'fire') mode.value = 1
        else if (newType === 'grass' || newType === 'bug') mode.value = 2
        else mode.value = 0 // default water-like
